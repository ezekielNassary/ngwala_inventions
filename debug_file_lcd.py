...
class App:
    ...
    def subMenuEvent3(self):
        ...
        tt = 0
        self.lcr()
        self.printtxt(0, 0, "Chaji Simu Yako")
        self.printtxt(0, 1, "Dakika: %.2f " % (bal))
        while loop:
            self.wdt.feed()
            try:
                if time.time() - self.previous >= 1:
                    ...
                    if tt == 60:
                        tmin += 1
                        tt = 0
                    self.printtxt(0, 2, "Dakka: %.0f " % (tmin))
                    self.printtxt(0, 3, "Sek  : %.0f " % (tt))
                    if tmin >= bal:
                        self.charge(0)
                        self.lcr()
